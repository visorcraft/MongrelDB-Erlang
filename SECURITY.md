...
This document describes the security properties of the MongrelDB Erlang client
and how to report vulnerabilities.

## Overview

The MongrelDB Erlang client is a pure-Erlang application (no native extensions)
that talks to `mongreldb-server` over HTTP using the standard library
`httpc` (inets). The client itself holds no encryption keys and stores no data
...
- **WARNING - raw SQL:** The `sql/2` function sends a raw SQL string to the
...
- Idempotency keys are caller-supplied opaque binaries; the client does not
...
- Server and network errors are mapped to the typed exception hierarchy
  ...
  status and decoded server envelope, not leaked as generic exceptions.
- NaN and Infinity raise a clear `mongreldb_query_error` instead of
...
The MongrelDB Erlang client has no runtime dependencies beyond the Erlang/OTP
...
- the MongrelDB Erlang client version, OTP version, and OS,
